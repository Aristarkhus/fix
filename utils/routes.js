	...
	appRoutes(){
		...
		this.app.post('/registerUser', async (request,response) => {
			...
			if(data.username === '') {
	            ...
	            registrationResponse.message = `Username masih kosong.`;
	            response.status(412).json(registrationResponse);
	        }else if(data.password === ''){
	            registrationResponse.error = true;
	            registrationResponse.message = `Password masih kosong.`;
	            response.status(412).json(registrationResponse);
	        }else{
				const result = await helper.registerUser( data );
				if (result === null) {
					registrationResponse.error = true;
					registrationResponse.message = `Registrasi gagal.`;
					response.status(417).json(registrationResponse);
				} else {
					registrationResponse.error = false;
					registrationResponse.userId = result.insertId;
					registrationResponse.message = `Hore!!. Registrasi Berhasil.`;
		...
		this.app.post('/login',async (request,response) =>{
			...
			if(data.username === '' || data.username === null) {
	            ...
	            loginResponse.message = `Username tidak boleh kosong.`;
	            response.status(412).json(loginResponse);
	        }else if(data.password === '' || data.password === null){
	            loginResponse.error = true;
	            loginResponse.message = `Password tidak boleh kosong.`;
	            response.status(412).json(loginResponse);
	        }else{
				const result = await helper.loginUser(data);
				if (result === null || result.length === 0) {
					loginResponse.error = true;
					loginResponse.message = `Username dan Password tidak sesuai.`;
